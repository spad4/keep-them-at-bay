...
local function reset()
    ...
    Discovered_Grenades = { ["frag_grenade"] = false }
    Grenade_Inventory = {}
    Grenades = {}
    Wall_Damage = 0
    Repair_Cost = 0
    for i = 1, 20 do
        Walls[i] = 100
        Wall_Flipped[i] = math.random() > 0.5
    end
    for i = 1, 16 do
        Turrets[i] = false
    end
    dandelion.ClearAll()
    gfx.shader_set(nil)
    effect.flash(2, gfx.COLOR_BLACK)
end
...
local function spawn_zombie(type, x, y, on_wall)
    ...
    new_zombie.invis_cutoff = model.invis_cutoff or 0
    ...
    new_zombie.chill_time = 0
    new_zombie.burn_time = 0
...
local function get_all_in_range(range, wall_allowed)
    local can_hit = {}
    for _, zombie in pairs(Zombies) do
        if wall_allowed or (zombie.y <= WALL_START - 40 and not zombie.on_wall) then -- zombies past the wall cannot be shot
            local rect = { x = zombie.x + ZOMBIE_W_ADJUST, y = zombie.y + ZOMBIE_H_ADJUST, w = zombie.w, h = zombie.h }
            if util.circ_rect_overlap(range, rect) then
                table.insert(can_hit, zombie)
            end
    ...
    for i = #Zombies, 1, -1 do
        ...
        if zombie.health <= 0 then
            ...
            if zombie.kb_x ~= 0 then -- weapon stun
                ...
                zombie.kb_x = util.approach(zombie.kb_x, 0, amount * 4)
            ...
            if zombie.kb_y ~= 0 then -- weapon stun
                ...
                zombie.kb_y = util.approach(zombie.kb_y, 0, amount * 4)
                ...
                if zombie.on_wall then
                    ...
                    if (wall1 or wall2) and string.match(zombie.type, "boomer") then
                        ...
                        if wall1 then
                            ...
                            Walls[wall1] = 0
                            dandelion.hit_wall((wall1 - 1) * 16 + 8, 160)
                            if Walls[wall1] == 0 then
                                conditional_screen_shake(0.5, 0.75)
                                shook_screen = true
                                dandelion.break_wall((wall1 - 1) * 16 + 8, 160)
                            end
                        end
                        if wall2 then
                            Wall_Damage += Walls[wall2]
                            Walls[wall2] = 0
                            dandelion.hit_wall((wall2 - 1) * 16 + 8, 160)
                            if Walls[wall2] == 0 then
                                conditional_screen_shake(0.5, 0.75)
                                shook_screen = true
                                dandelion.break_wall((wall2 - 1) * 16 + 8, 160)
                            end
                        end
                    end
                    if wall1 then
                    ...
                    if wall2 and wall1 ~= wall2 then
                        ...
                        if Walls[wall2] == 0 then
                            conditional_screen_shake(0.5, 0.75)
                            shook_screen = true
                            dandelion.break_wall((wall2 - 1) * 16 + 8, 140)
                        end
                    end
                    if not wall1 and not wall2 then
                        ...
                        if not shook_screen then
                            conditional_screen_shake(0.25, 0.15)
                        end
                    end
                    zombie.last_move = elapsed
                    if (Elapsed - zombie.chill_time < CHILL_DURATION) then
                        zombie.last_move += zombie.move_delay * 1.5
                    end
                end
                zombie.moved = true
...
local function do_waves()
    ...
    if Elapsed - Transition_Started > next_spawn.time then
...
local function insert_drawer_by_order(item)
    ...
    table.insert(Drawer_Items, item)
end
...
local function do_turrets()
    for i, turret in pairs(Turrets) do
        ...
        if Elapsed - turret.cooldown < turret.fire_rate then goto continue end
        ...
        local vec = util.vec_from_angle(angle, 22)
        local adjusted_spread = turret.spread * 0.01

        for _ = 1, turret.count do
            local spread = math.random() * adjusted_spread - adjusted_spread / 2
            ...
            if turret.bullet == "rifle" or turret.bullet == "shell" or turret.bullet == "sniper" then
                ...
                if zombie then
                    ...
                    dandelion.hitscan_bullet(start_x, start_y,
                        { ["config"] = { length = distance, rotation = angle + spread, thickness = BULLETS[turret.bullet].thickness } })
                else
                    dandelion.hitscan_bullet(start_x, start_y,
                        { ["config"] = { length = 1000, rotation = angle + spread, thickness = BULLETS[turret.bullet].thickness } })
                end

                if turret.bullet == "sniper" or turret.bullet == "shell" then
                    dandelion.sniper_smoke_emitter(start_x, start_y)
                    dandelion.bullet_sparks_emitter(start_x, start_y, { rotation = (angle + spread) / math.pi })
                end
                dandelion.small_smoke(start_x, start_y)
            elseif turret.bullet == "ice" then
...
local function do_grenades()
    for i = #Grenades, 1, -1 do
        ...
        grenade.age = Elapsed - grenade.born

        if grenade.id == "molotov_fire" then
            if Elapsed - grenade.born > (Weather == "rain" and 5 or 10) then
                table.remove(Grenades, i)
            end

            local targets = get_all_in_range({ x = grenade.x, y = grenade.y, r = 32 }, true)
            if targets then
                for _, zombie in pairs(targets) do
                    zombie.burn_time = Elapsed
                end
            end
        else
            ...
            if grenade.age > duration then
                ...
                if grenade.id == "frag_grenade" then
                    ...
                    if targets then
                        for _, zombie in pairs(targets) do
                            local dx, dy = zombie.x + ZOMBIE_W_ADJUST - grenade.dx,
                                zombie.y + ZOMBIE_H_ADJUST - grenade.dy
                            local h = util.vec_dist({ x = 0, y = 0 }, { x = dx, y = dy })
                            local theta = math.asin(dy / h)
                            local angle = dx > 0 and theta or (math.pi - theta)
                ...
                elseif grenade.id == "molotov" then
                    local fire = {
                        id = "molotov_fire",
                        x = grenade.dx,
                        y = grenade.dy,
                    ...
                    table.insert(Grenades, fire)
                    dandelion.molotov_break(grenade.dx, grenade.dy)
                    if Weather == "rain" then
                        dandelion.short_molotov_fire(grenade.dx, grenade.dy)
                    else
                        dandelion.molotov_fire(grenade.dx, grenade.dy)
                    end
                end
            end
        end
...
function _update(dt)
    ...
    if Reloading and Elapsed - Reload_Start > Weapon.reload then
        Reloading = false
        Ammo = Weapon.ammo
    end
    ...
    if Is_Night then
        ...
        if (Elapsed - Last_Regen > Regen_Timer) and Health_Regen > 0 and Player.health < Player_Health then
            Last_Regen = Elapsed
    ...
    if input.key_held(input.KEY_H) and usagi.IS_DEV then
        spawn_zombie("sprinter_3", 160, 0)
    end

    if input.key_pressed(input.KEY_C) and usagi.IS_DEV then
        -- damage_player(5)
        change_money(250)
    end
    ...
    if input.key_pressed(input.KEY_U) and usagi.IS_DEV then
        Zombies = {}
        Current_Wave = {}
    end
    ...
    if Is_Night then
        ...
        if input.mouse_held(input.MOUSE_LEFT) then
            if Elapsed - Fire_Cooldown > Weapon.fire_rate then
                if Ammo > 0 then
                    shoot()
                end
        ...
        if Choice_1 and Choice_2 then
                ...
                if Highlighted_Choice and Choice_Progression > 2 then
                    ...
                    if Choice_Type == "mutation" then
                        ...
                        if choice.next and ZOMBIES[choice.next] then
                            table.insert(Potential_Mutations, math.random(1, math.floor(#Potential_Mutations) or 1),
                                choice.next)
                        end
                    elseif Choice_Type == "unlock" then
                        ...
                        if choice.next and UNLOCKS[choice.next] then
                            table.insert(Potential_Unlocks, math.random(1, math.floor(#Potential_Unlocks) or 1),
                                choice.next)
                        end
                    end
            ...
            if Drawer then
                ...
                if Selected_Turret and Turrets[Selected_Turret] and input.key_pressed(input.KEY_G) then
                    change_money(math.floor(Turrets[Selected_Turret].cost / 2))
                    Turrets[Selected_Turret] = false
            ...
            if not Drawer and not Choice_1 and Repair_Cost > 0 and Money >= Repair_Cost and input.key_pressed(input.KEY_G) then
...
local function draw_zombies()
    for i, zombie in pairs(Zombies) do
        if Elapsed - zombie.burn_time < FIRE_DURATION and math.random() > 0.9 then
            dandelion.on_fire(zombie.x + ZOMBIE_W_ADJUST + 4, zombie.y + ZOMBIE_H_ADJUST + 4)
            zombie.health -= 0.5
        end
        if zombie.on_wall then
            table.insert(WallZombies, zombie)
        elseif zombie.moved then
            if not string.match(zombie.type, "stalker") or zombie.y > zombie.invis_cutoff then
                local tint = (Elapsed - zombie.chill_time < CHILL_DURATION) and gfx.COLOR_BLUE or gfx.COLOR_TRUE_WHITE
                ...
                gfx.sspr_ex(zombie.spr_x + zombie.variant * H_SIZE, zombie.spr_y + zombie.current_frame * H_SIZE, H_SIZE,
                    H_SIZE, zombie.x - 2 * (zombie.scale - 1), zombie.y - 2 * (zombie.scale - 1), H_SIZE * zombie.scale,
                    H_SIZE * zombie.scale,
...
local function draw_player()
    ...
    local m = Elapsed - Fire_Cooldown < (0.025 + Weapon.recoil / 16) and -1 * math.max(Weapon.recoil, 1) or 0
...
local function draw_crosshair()
    ...
    local elapsed = Elapsed - Fire_Cooldown
...
local function draw_hud()
    ...
    -- local hp_color = Elapsed - Player.last_hit > 0.125 and gfx.COLOR_RED or gfx.COLOR_WHITE
    ...
    if Elapsed - Player.last_hit < 0.25 then
...
local function draw_turrets()
    for i, turret in pairs(Turrets) do
        if turret then
            ...
            local amount = 1 - (Elapsed - turret.cooldown) / 0.05
            local m = (Elapsed - turret.cooldown < 0.05 and -amount or 0) * turret.recoil
...
local function draw_walls()
    for i = 1, #Walls do
        ...
        gfx.sspr_ex(offset * H_SIZE, 224, H_SIZE, H_SIZE, (i - 1) * H_SIZE, 148, H_SIZE, H_SIZE, Wall_Flipped[i], false,
            0, gfx.COLOR_TRUE_WHITE, 1)
        gfx.sspr_ex(offset * H_SIZE, 240, H_SIZE, H_SIZE, (i - 1) * H_SIZE, 164, H_SIZE, H_SIZE, Wall_Flipped[i], false,
            0, gfx.COLOR_TRUE_WHITE, 1)
    end
end
...
local function draw_drawer()
    ...
    if Drawer_Type == "turret" and Selected_Turret and Turrets[Selected_Turret] then
        text_with_shadow("($" .. (math.floor(Turrets[Selected_Turret].cost / 2)) .. ") Sell Turret [G]", 128, 104,
            gfx.COLOR_RED, gfx.COLOR_DARK_PURPLE)
    end
    ...
    for i, item in pairs(Drawer_Items) do
        ...
        if item then
            ...
            elseif Drawer_Type == "weapon" then
                local tint = ((Money >= item.cost or Discovered_Weapons[item.id]) and Weapon.id ~= item.id) and
                    gfx.COLOR_TRUE_WHITE or gfx.COLOR_DARK_GRAY
                gfx.sspr_ex(item.spr_x, item.spr_y, 16, 16, x - 1, y - 2, 32, 32, false, false, 0, tint, 1)
            ...
            elseif Drawer_Type == "grenade" then
                ...
                local tint = (Money >= item.cost and #Grenade_Inventory < Grenade_Slots) and gfx.COLOR_TRUE_WHITE or
                    gfx.COLOR_DARK_GRAY
                gfx.sspr_ex(item.spr_x, item.spr_y, 14, 14, x + 3, y + 3, 28, 28, false, false, 0, tint, 1)
...
local function draw_choices()
    local elapsed = Elapsed - Transition_Started
    local w = 144
    local h = 100
    local text_1 = ""
    local text_2 = ""
    ...
    if Choice_Type == "unlock" then
        text_1 = "Your technology advances."
...
local function draw_grenade_inventory()
    ...
    if Is_Night and #Grenade_Inventory > 0 then
        ...
        text_with_shadow("[G]", offset + 12, 166, gfx.COLOR_WHITE, gfx.COLOR_DARK_GRAY)
    end
end
...
function _draw(dt)
    ...
    if Is_Night then
        local alpha = math.min(max + (Elapsed - Transition_Started), 0.6)
        gfx.sspr_ex(496, 496, 16, 16, 0, 0, 320, 180, false, false, 0, gfx.COLOR_DARK_BLUE, alpha)
    else
        local alpha = math.max(0.6 - (Elapsed - Transition_Started), max)
        gfx.sspr_ex(496, 496, 16, 16, 0, 0, 320, 180, false, false, 0, gfx.COLOR_DARK_BLUE, alpha)
    end
    ...
    if not Is_Night and Player.x < 10 and not Choice_1 then
        text_with_shadow("[SPACE] Start Night", 2, 104, gfx.COLOR_RED, gfx.COLOR_DARK_PURPLE)
    end
